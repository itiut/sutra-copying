...
int main(int argc, char **argv) {
    ...
    fprintf(out, "Here it is, with each space-delimited element on a line (with commentary on the first character):<br>");
    ok_array *spaced = ok_array_new(ucs, " \n");
    for (int i = 0; i < spaced->length; i++) {
        fprintf(out, "%s", spaced->elements[i]);
        gunichar c = g_utf8_get_char(spaced->elements[i]);
