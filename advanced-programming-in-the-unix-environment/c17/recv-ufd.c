...
#define CREDSLEN CMSG_LEN(siezof(struct CREDSTRUCT))
...
int recv_ufd(int fd, uid_t *uidptr, ssize_t (*userfunc)(int, const void *, size_t)) {
    ...
    if (setsockopt(fd, SOL_SOCKET, CREDOPT, &on, sizeof(int)) == -1) {
    ...
    for (;;) {
        ...
        if ((nr = recvmsg(fd, &msg, 0)) == -1) {
            err_ret("recvmsg error");
            return -1;
        } else if (nr == 0) {
            err_ret("conection closed by server");
            return -1;
        }
