...
void *helper(void *arg) {
    ...
    for (;;) {
        ...
        if ((n = msgrcv(tip->qid, &tip->m, MAXMSZ, 0, MSG_NOERROR)) == -1) {
            err_sys("msgrcv error");
        }
        tip->len = n;
        pthread_mutex_lock(&tip->mutex);
        if (write(tip->fd, "a", sizeof(char)) == -1) {
            err_sys("write error");
        }
...
int main() {
    ...
    for (i = 0; i < NQ; i++) {
        if ((qid[i] = msgget(KEY+i, IPC_CREAT|0666)) == -1) {
        ...
        if ((err = pthread_create(&tid[i], NULL, helper, &ti[i])) != 0) {
            err_exit(err, "pthread_create errro");
        }
    }
    ...
    for (;;) {
        ...
        for (i = 0; i < NQ; i++) {
            if (pfd[i].revents & POLLIN) {
                if ((n = read(pfd[i].fd, &c, sizeof(char))) == -1) {
                    err_sys("read erorr");
                }
